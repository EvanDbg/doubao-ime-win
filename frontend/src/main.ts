...
type RawKey = { vk_code: number; scan_code?: number | null; extended: boolean };
type Config = {
  general: { auto_start: boolean; language: string };
  hotkey: { action: string; binding: string; mode: string; standard_key: string; double_tap_interval_ms: number; raw_key?: RawKey | null };
...
const field = (id: string, label: string, value: unknown, type = "text", attrs = "") => `<label class="field"><span>${label}</span><input id="${id}" type="${type}" value="${esc(value)}" ${attrs}/></label>`;
...
function renderSettings() {
  ...
  const standardKey = c.hotkey.standard_key;
  ...
  <section class="settings-page" id="hotkeys"><h2>热键配置</h2><div class="grid two">${select("hotkey_action", "触发动作", c.hotkey.action || "voice_input", [["voice_input", "本程序语音输入"], ["official_hold", "官方豆包：长按（右 Alt）"], ["official_hands_free", "官方豆包：免按（左 Ctrl + Win）"]])}<div data-hotkey-action="voice_input">${select("hotkey_type", "按键类型", hotkeyType, [["standard", "标准按键"], ["raw", "非标准按键"]])}</div><div data-hotkey-action="voice_input">${select("trigger_mode", "触发类型", triggerMode, [["single_tap", "单击"], ["double_tap", "双击"], ["hold", "长按"]])}</div><div data-hotkey-action="voice_input" data-hotkey-type="standard">${field("standard_key", "触发按键", standardKey)}</div><div data-hotkey-action="voice_input" data-trigger-mode="double_tap">${field("double_tap_interval", "双击间隔（毫秒）", c.hotkey.double_tap_interval_ms, "number", 'min="100" max="1000" step="50"')}</div></div><div class="row" data-hotkey-type="raw"><button class="secondary" id="capture">录入非标准按键</button><span id="capture-status">${c.hotkey.raw_key ? `已录入：键码 ${c.hotkey.raw_key.vk_code} / 扫描码 ${c.hotkey.raw_key.scan_code ?? "任意"}` : "可录入小爱键、媒体键等厂商按键"}</span></div><div class="row" data-hotkey-action="official_hold"><span>按住自定义按键说话，松开结束。触发时转发官方右 Alt 快捷键，本程序不录音、不弹出悬浮窗；该按键将由本程序独占。</span></div><div class="row" data-hotkey-action="official_hands_free"><span>按一下开始说话，再按一下结束。触发时转发官方左 Ctrl + Win 快捷键，本程序不录音、不弹出悬浮窗；该按键将由本程序独占。</span></div></section>
...
const PURE_MODIFIERS = ["ctrl", "control", "shift", "alt", "win", "super", "meta"];
function validatedStandardKey(): string | null {
  const key = value("standard_key").trim();
  if (!key) { alert("请填写触发按键，例如 Ctrl+Shift+V 或 F13"); return null; }
  const parts = key.split("+").map(part => part.trim());
  if (parts.some(part => !part)) { alert("触发按键格式无效：包含空的按键段"); return null; }
  if (parts.length > 1 && PURE_MODIFIERS.includes(parts[parts.length - 1].toLowerCase())) {
    alert("组合键的最后一段需要是普通按键，例如 Ctrl+Shift+V"); return null;
  }
  return parts.join("+");
}
// Only visible fields are read back: a hidden standard-key input must not
// overwrite the stored value while a raw binding is active.
function formHotkeyConfig(): Config["hotkey"] | null {
  if (!config) return null;
  const action = value("hotkey_action");
  const hotkey = { ...config.hotkey, action };
  if (action === "voice_input") {
    hotkey.binding = value("hotkey_type");
    hotkey.mode = value("trigger_mode");
  } else {
    hotkey.binding = "raw";
    hotkey.mode = action === "official_hold" ? "hold" : "single_tap";
  }
  if (hotkey.binding === "raw") {
    if (!hotkey.raw_key) { alert("请先通过“录入非标准按键”录制一个按键"); return null; }
  } else {
    const key = validatedStandardKey();
    if (!key) return null;
    hotkey.standard_key = key;
  }
  if (hotkey.mode === "double_tap") {
    const interval = Math.round(Number(value("double_tap_interval")));
    if (!Number.isFinite(interval)) { alert("双击间隔需要是 100-1000 之间的毫秒数"); return null; }
    hotkey.double_tap_interval_ms = Math.min(1000, Math.max(100, interval));
  }
  return hotkey;
}
function save() {
  if (!config) return;
  const hotkey = formHotkeyConfig();
  if (!hotkey) return;
